...
export default function BillingPage() {
  ...
  const [paymentMode, setPaymentMode] = useState("paystack"); // 'paystack' or 'simulated'
  ...
  const handleSimulatedRecharge = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        toast.success(`Sandbox Success! Credited KSh ${amount.toFixed(2)} to your wallet.`);
        setBilling(data.data);
        setRechargeAmount("");
      } else {
  ...
  const handlePaystackPayment = () => {
    const amount = parseFloat(rechargeAmount);
    if (isNaN(amount) || amount <= 0) {
      return toast.warning("Please enter a valid amount to recharge");
    }

    if (!window.PaystackPop) {
      return toast.error("Paystack Checkout SDK is still loading. Please retry in a few seconds.");
    }

    const authData = localStorage.getItem("auth");
    const email = authData ? JSON.parse(authData).user?.email : "customer@sms-saas.com";

    // Paystack amounts are processed in kobos/cents (multiply by 100)
    const amountInKobos = Math.round(amount * 100);

    // Paystack Public Key
    const paystackPublicKey =
      process.env.NEXT_PUBLIC_PAYSTACK_PUBLIC_KEY || "pk_test_418702c2dbf11b66df870f7d9a0d4215ad14db9a";

    const handler = window.PaystackPop.setup({
      key: paystackPublicKey,
      email: email,
      amount: amountInKobos,
      currency: "KES", // Kenyan Shillings
      callback: async (response) => {
        toast.info("Payment processed by Paystack. Verifying transaction reference...");
        try {
          setActionLoading(true);
          const res = await fetch(`${API_BASE_URL}/billing/paystack`, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${localStorage.getItem("token")}`,
          ...
          const data = await res.json();
          if (data.success) {
            toast.success(`Paystack Verified! KSh ${amount.toFixed(2)} credited to your balance.`);
            setBilling(data.data);
            setRechargeAmount("");
          } else {
      ...
      onClose: () => {
        toast.warning("Paystack Checkout popup dismissed.");
      },
    });
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto pb-12">
      {/* Paystack Inline Checkout JS Loader */}
          ...
          <Card className="border-0 shadow-lg rounded-3xl bg-white dark:bg-gray-900 border-t-4 border-emerald-600 transition duration-300 hover:shadow-xl">
            <CardContent className="p-6 md:p-8 space-y-6">
              <div className="flex items-center justify-between gap-4 border-b pb-4 flex-wrap">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-gray-500 text-xs dark:text-gray-400">Add credits to fund Pay As You Go dispatches</p>
                  </div>
                </div>

                {/* MODE CHANGER */}
                <div className="flex bg-gray-100 dark:bg-gray-800 p-0.5 rounded-xl text-xs font-bold border dark:border-gray-700">
                  <button
              ...
              <form onSubmit={handleDepositSubmit} className="space-y-4">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3">
                    ...
                    >
                      {actionLoading ? "Processing..." : (
                        <>
                          <CreditCard size={16} />
                          {paymentMode === "paystack" ? "Pay with Paystack" : "Credit (Sandbox)"}
                        </>
                      )}
                    </Button>
                  </div>
                ...
                {paymentMode === "paystack" ? (
                  <div className="flex gap-2.5 items-start bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-100/60 dark:border-emerald-900/40 rounded-2xl p-4.5 text-xs text-emerald-800 dark:text-emerald-300 leading-relaxed font-semibold">
                    <AlertCircle size={16} className="shrink-0 mt-0.5" />
                    <span>
                      Paystack supports M-Pesa mobile money and major credit/debit cards in Kenya. Funds are verified instantly by our server and credited to your wallet.
                    </span>
                  </div>
                ) : (
                  <div className="flex gap-2.5 items-start bg-amber-50 dark:bg-amber-950/20 border border-amber-100/60 dark:border-amber-900/40 rounded-2xl p-4.5 text-xs text-amber-800 dark:text-amber-300 leading-relaxed font-semibold">
                    <AlertCircle size={16} className="shrink-0 mt-0.5" />
                    <span>
                      Sandbox mode executes simulated credits. Use this during testing to instantly load balance without spending real money.
                    </span>
                  </div>
                )}
              </form>
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <Card className="border-0 shadow-lg rounded-3xl bg-white dark:bg-gray-900 transition duration-300 hover:shadow-xl h-full flex flex-col justify-between">
            <CardContent className="p-6 md:p-8 space-y-6">
              ...
              <div className="space-y-4 text-xs font-semibold text-gray-600 dark:text-gray-400 leading-relaxed font-medium">
                <p>
                ...
                <div className="bg-gray-50 dark:bg-gray-950/40 p-4.5 rounded-2xl border border-gray-100 dark:border-gray-850 space-y-3 text-gray-850 dark:text-gray-200 font-bold">
                  <span className="text-[10px] text-gray-400 uppercase tracking-wider block font-bold">Example Scenario</span>
                ...
                <div className="flex gap-2.5 items-start bg-amber-50 dark:bg-amber-950/20 border border-amber-100/60 dark:border-amber-900/40 rounded-2xl p-4.5 text-amber-800 dark:text-amber-300 leading-normal font-semibold">
                  <AlertCircle size={16} className="shrink-0 mt-0.5" />
                  <span>
                    Scheduled campaigns check your available balance right before triggering. Keep your balance funded to avoid background send failures.
                  </span>
                </div>
              </div>
            </CardContent>
